...
class CrowdStrikeAdapter(BaseAdapter):
    ...
    async def fetch_raw(self) -> List[Dict]:
        """Devices come back fully hydrated from one combined endpoint -- no query-then-hydrate
        step needed there. Users are the opposite: queryUserV1 returns UUIDs only, and per-user
        role assignments (CombinedUserRolesV2) are confirmed to take a single user_uuid -- no
        batch-roles endpoint exists, so that N+1 is the API's actual shape, not a design mistake
        (same situation as Slack's per-channel history calls). User DETAIL fetching here is also
        done per-UUID rather than batched: CrowdStrike's docs strongly imply a batch entities
        endpoint exists (matching the devices pattern), but the exact path wasn't confirmed against
        real docs content, so this uses the one confirmed-safe path per user instead of guessing
        at an unverified batch endpoint -- worth revisiting once verified against a live account."""
        device_params = {"limit": 500}
        if self.config.device_filter:
            device_params["filter"] = self.config.device_filter

        devices = await self.client.paginated_get(
        ...
        user_uuids = await self.client.paginated_get(
        ...
        for uuid in user_uuids:
            ...
            roles = [
                r.get("role_name") or r.get("role_id")
                for r in roles_resp.json().get("resources", [])
            ]
